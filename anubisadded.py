...
class UI(QMainWindow):
    ...
    def intUI(self):
        ...
        # I have three menu items aded forth
        filemenu = menu.addMenu('File')
        Port = menu.addMenu('Port')
        Run  = menu.addMenu('Run')
        Exct = menu.addMenu('Exct')
        ...
        maln_Auction = QAction("Main_looks like",self)
        maln_Auction.triggered.connect(self.mainlook)
        Exct.addAction( maln_Auction)

        Exct_Auction = QAction("create test",self)
        Exct_Auction.triggered.connect(self.Exct)
        Exct.addAction( Exct_Auction)

        run_Auction = QAction("run",self)
        run_Auction.triggered.connect(self.run)
        Exct.addAction( run_Auction)
        
        Clr_Auction = QAction("Clr all",self)
        Clr_Auction.triggered.connect(self.Clr)
        Exct.addAction( Clr_Auction)
...
#        Port.addAction(Port_Action)
    ...
    def mainlook(self):
       text.setText("def main():")
       text.append("#call the function and the parameters here# ")
       text.append("def #funcition name#(parameters if there):")
       text.append("# define the function here #")
       a = 'if __name__== "__main__":'
       text.append(a)
       text.append(" main()")
       text2.append("after finishing click Exct to run  ")
    def Exct(self):
   	  ...
   	  f.write(s)
   	  f.close
   	  text2.append("file is ready to run   ")
   	  
   	  
   	         
    def run(self):
        try:
            exec(open('demofile2.py').read())
            text2.append("file is running  see results clear to start over   ")
        except:
            text.setText("")
            text2.append("file has an error clear and start over   ")   
    def Clr(self):
        try:
